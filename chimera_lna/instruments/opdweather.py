...
class OpdWeather(
    WeatherBase, WeatherTemperature, WeatherHumidity, WeatherPressure, WeatherWind
):
    __config__ = {
    ...
    def __init__(self):
        ...
        self._angles = np.arange(0, 360, 360.0 / len(self._directions))
        ...
        try:
            url = urllib.request.urlopen(
                urllib.request.Request(
                    "http://200.131.64.185/clima/download.txt",
                    headers={"Range": "bytes=-300"},
                )
            )
        except urllib.error.URLError as e:
            self.log.error(f"Error opening url {uri}: {e}")
            return False
        (
            date_ws,
            time_ws,
            ...
            pressure,
            rain,
        ) = np.array(self.field_re.split(url.readlines()[-1].strip()))[
            [0, 1, 2, 5, 6, 7, 8, 15, 16]
        ]
        url.close()
    ...
    def _check(self):
        if time.time() >= self._last_check + self["check_interval"]:
            ...
            try:
                ...
                ValueError
            ):  # Sometimes the data returned from WS is "---". I will ingore it.
                return False
        else:
            return True
    ...
    def temperature(self, unit_out=units.Celsius):
        ...
        return WSValue(
            self.obs_time(),
            self._convert_units(self._temperature, units.Celsius, unit_out),
            unit_out,
        )
    ...
    def wind_speed(self, unit_out=units.meter / units.second):
        return WSValue(
            self.obs_time(),
            self._convert_units(self._wind_speed, (units.km / units.h), unit_out),
            unit_out,
        )
    ...
    def dew_point(self, unit_out=units.Celsius):
        return WSValue(
            self.obs_time(),
            self._convert_units(self._dew_point, units.deg_C, unit_out),
            unit_out,
        )
    ...
    def pressure(self, unit_out=units.Pa):
        return WSValue(
            self.obs_time(),
            self._convert_units(self._pressure, units.bar / 1000, unit_out),
            unit_out,
        )
    ...
    def getMetadata(self, request):
        ...
        return [
            ("ENVMOD", str(self["model"]), "Weather station Model"),
            (
                "ENVTEM",
                self.temperature(unit_out=units.deg_C).value,
                "[degC] Weather station temperature",
            ),
            (
                "ENVHUM",
                self.humidity(unit_out=units.pct).value,
                "[%] Weather station relative humidity",
            ),
            (
                "ENVWIN",
                self.wind_speed(unit_out=units.m / units.s).value,
                "[m/s] Weather station wind speed",
            ),
            (
                "ENVDIR",
                self.wind_direction(unit_out=units.deg).value,
                "[deg] Weather station wind direction",
            ),
            (
                "METDEW",
                self.dew_point(unit_out=units.deg_C).value,
                "[degC] Weather station dew point",
            ),
            (
                "ENVPRE",
                self.pressure(unit_out=units.cds.mmHg).value,
                "[mmHg] Weather station air pressure",
            ),
            # ('METRAIN', str(self.pressure()), 'Weather station rain indicator'),
            (
                "ENVDAT",
                self.obs_time().strftime("%Y-%m-%dT%H:%M:%S.%f"),
                "Date of the meteo observation",
            ),
        ]
